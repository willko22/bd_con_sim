...
template <typename T> struct Color
{
    ...
    {
        return Color<float>(
            static_cast<float>(r) * inv255, static_cast<float>(g) * inv255,
            static_cast<float>(b) * inv255, static_cast<float>(a) * inv255);
    }
    ...
    {
        return Color<T>(static_cast<T>(r * factor), static_cast<T>(g * factor),
                        static_cast<T>(b * factor), static_cast<T>(a * factor));
    }
...
namespace obj
{
    ...
    struct Vec2
    {
        ...
        constexpr Vec2(i16 x, i16 y) noexcept
            : x(static_cast<float>(x)), y(static_cast<float>(y))
        {
        }
        ...
        Vec2 ceil() const noexcept { return Vec2(std::ceil(x), std::ceil(y)); }
    ...
    struct Vec2Hash
    {
        ...
        {
            // Convert floats to integers for hashing (with reasonable
            // precision)
            const u32 x_hash =
                std::hash<float>{}(std::round(v.x * 1000.0f) / 1000.0f);
            const u32 y_hash =
                std::hash<float>{}(std::round(v.y * 1000.0f) / 1000.0f);
    ...
    struct Vec2Set
    {
        ...
        explicit Vec2Set(size_t capacity) { points.reserve(capacity); }

        void add(float x, float y) noexcept { points.emplace(x, y); }
        ...
        void add(const Vec2 &v) noexcept { points.insert(v); }
    ...
    struct Vec2List
    {
        ...
        explicit Vec2List(size_t capacity) { points.reserve(capacity); }
        ...
        void add(const Vec2 &v) { points.push_back(v); }

        Vec2 &operator[](size_t index) noexcept { return points[index]; }
        ...
        size_t capacity() const noexcept { return points.capacity(); }
    ...
    struct BBox
    {
        ...
        float getArea() const noexcept { return width * height; }

        // Expand bounding box by a margin
        BBox expand(float margin) const noexcept
        {
            return BBox(x - margin, y - margin, width + 2 * margin,
                        height + 2 * margin);
        }
        ...
        {
            return BBox(std::floor(x), std::floor(y), std::ceil(width),
                        std::ceil(height));
        }
        ...
        i16 getIntX() const noexcept { return static_cast<i16>(std::round(x)); }
        i16 getIntY() const noexcept { return static_cast<i16>(std::round(y)); }
        u16 getIntWidth() const noexcept
        {
            return static_cast<u16>(std::round(width));
        }
        u16 getIntHeight() const noexcept
        {
            return static_cast<u16>(std::round(height));
        }
    };
    ...
    struct BCircle
    {
        ...
        BCircle(const Vec2 &center, float radius) noexcept
            : center(center), radius(radius)
        {
            setRadius(radius);
        }
        BCircle(i16 x, i16 y, float radius) noexcept
        ...
        // Optimized circle intersection with early exits and branch prediction
        // hints
        bool intersects(const BCircle &other) const noexcept
        {
            ...
            // Early exit: Manhattan distance check (very fast, eliminates ~50%
            // of cases)
            const float dx_abs = std::abs(other.center.x - center.x);
            const float dy_abs = std::abs(other.center.y - center.y);

            // Quick rejection: if Manhattan distance > combined radius *
            // sqrt(2), definitely no intersection
            if (dx_abs + dy_abs > combined_radius * 1.414213f)
            {
                return false;
            }

            // Quick acceptance: if both dx and dy are small, definitely
            // intersecting
        ...
        // Check if circle is completely within screen bounds (fast screen
        // culling)
        inline bool isOnScreen(u16 screen_width,
                               u16 screen_height) const noexcept
        {
        ...
        inline void scale(float factor) noexcept { setRadius(radius * factor); }

        // Get bounding box for this circle (useful for spatial partitioning)
        inline BBox getBBox() const noexcept
        {
            return BBox(center.x - radius, center.y - radius, radius * 2.0f,
                        radius * 2.0f);
        }
        ...
        inline bool isOnEdge(const Vec2 &point,
                             float tolerance = 1.0f) const noexcept
        {
            const float distSquared = distanceSquaredTo(point);
            const float inner_radius =
                (radius > tolerance) ? radius - tolerance : 0.0f;
            ...
            return distSquared >= inner_radius_sq &&
                   distSquared <= outer_radius_sq;
        }
        ...
        // Check if circle intersects with a rectangle (useful for collision
        // with game obj)
        inline bool intersects(const BBox &bbox) const noexcept
        {
            // Find the closest point on the rectangle to the circle center
            const float closest_x =
                std::max(bbox.x, std::min(center.x, bbox.x + bbox.width));
            const float closest_y =
                std::max(bbox.y, std::min(center.y, bbox.y + bbox.height));
     ...
     * ---------------------------
     * The polygon supports three types of rotation using standard aviation
     * terminology:
     *
     ...
     *    - Positive yaw: "turn left" motion - object rotates counterclockwise
     * when viewed from above
     *    - Negative yaw: "turn right" motion - object rotates clockwise when
     * viewed from above
     *    - In 2D view: affects the X-coordinate more dramatically than Y
     *
     * 3. ROLL (rotation around Z-axis):
     *    - Positive roll: "bank right" motion - right side tilts down, left
     * side tilts up
     *    - Negative roll: "bank left" motion - left side tilts down, right side
     * tilts up
     *    - In 2D view: traditional 2D rotation, both X and Y coordinates change
     * equally
     *
     ...
     * - points_rotated: Points after applying all current rotations (used for
     * rendering)
     *
    ...
    struct Polygon
    {
        ...
        float pitch =
            0.0f; // 4 bytes (rotation around x-axis - nodding up/down)
        float yaw =
            0.0f; // 4 bytes (rotation around y-axis - turning left/right)
        float roll =
            0.0f; // 4 bytes (rotation around z-axis - tilting left/right)

        bool move =
            false; // 1 byte (indicates if polygon is moving, used for physics)
        bool should_rotate = false; // 1 byte (indicates if polygon can rotate,
                                    // used to disable rotation)
        bool should_render = true;  // 1 byte (indicates if polygon should be
                                    // rendered, used for visibility)

        float randPhase =
            0.0f; // 4 bytes (random phase for flutter effect, if needed)
        ...
        float spawn_time =
            0.0f; // 4 bytes - time when rectangle was spawned (in seconds)
        float stop_time =
            0.0f; // 4 bytes - time when rectangle was stopped (in seconds)

        // Physics properties
        Vec2 position; // 8 bytes - current position (duplicate of bbox.center
                       // for physics clarity)
        Vec2 velocity; // 8 bytes - velocity vector in world units per second
        Vec2 acceleration; // 8 bytes - acceleration vector in world units per
                           // second²
        float mass = 1.0f; // 4 bytes - mass for physics calculations

        // Legacy properties (keeping for compatibility)
        float speed =
            0.0f; // 4 bytes (deprecated - use velocity magnitude instead)
        float decay_rate = 0.0f; // 4 bytes - decay rate for time-based effects
                                 // (e.g., fading out)
        ...
        Polygon(const Color<u8> &c, i16 x = 0, i16 y = 0)
            : color(c),
              bbox(Vec2(static_cast<float>(x), static_cast<float>(y)), 0.0f)
        {
            // Initialize physics properties
        ...
        void applyForce(const Vec2 &force) { acceleration += force / mass; }

        // Apply impulse (immediate velocity change)
        void applyImpulse(const Vec2 &impulse) { velocity += impulse / mass; }
        ...
        void setVelocity(float vx, float vy) { setVelocity(Vec2(vx, vy)); }
        ...
        void updatePhysics(float dt)
        {
            ...
            // Reset acceleration for next frame (forces must be reapplied each
            // frame)
            acceleration = Vec2(0.0f, 0.0f);
        ...
        // === Legacy compatibility methods (deprecated but kept for existing
        // code) ===
        ...
        void adjustDirection(const Vec2 &v) { adjustDirection(v.x, v.y); }
        ...
        void setSpeed(float new_speed)
        {
            ...
            else
            {
                ...
                // Let the physics system handle initial velocity through
                // impulses
                velocity = Vec2(0.0f, 0.0f);
                direction =
                    Vec2(1.0f, 0.0f); // Keep legacy direction for compatibility
            }
            speed = new_speed;
        }

        void adjustSpeed(float delta_speed) { setSpeed(speed + delta_speed); }
        ...
        Color<float> getGLColor() const { return color.toGL(); }
        ...
        void addPoint(i16 x, i16 y)
        {
            addPoint(
                x, y,
                0); // Default mode: add to current position into both lists
        }
        ...
        // Add point with mode:
        // Mode 0: Add to current position into original and into rotated
        // version as rotated by current rotation Mode 1: Add to rotated as
        // current position and into original as rotated in reverse Mode 2: Add
        // to both as current position
        void addPoint(i16 x, i16 y, int mode)
        {
            ...
            case 0:
            {
                ...
                // Add to rotated as current position rotated by current
                // rotations
        ...
        const BCircle &getBBox() const { return bbox; }

        // Get the center point
        const Vec2 &getCenter() const { return bbox.center; }

        // Get current rotations for all axes with descriptive names
        // Pitch: Rotation around X-axis (nodding up/down) - positive pitch
        // tilts front up Yaw: Rotation around Y-axis (turning left/right) -
        // positive yaw turns left Roll: Rotation around Z-axis (tilting
        // left/right) - positive roll tilts right side down
        float getPitch() const { return pitch; }
        float getYaw() const { return yaw; }
        float getRoll() const { return roll; }

        // Get the original (non-rotated) points
        const Vec2List &getOriginalPoints() const { return points_original; }

        // Get the rotated points (these are what should be rendered)
        const Vec2List &getRotatedPoints() const { return points_rotated; }

        // Get the points for rendering (alias for getRotatedPoints for clarity)
        const Vec2List &getPoints() const { return points_rotated; }

        // Recalculate BCircle and center from current rotated points
        void updateBBox() { _calculateBBox(); }

        // Move polygon so its BCircle center is at specified position
        void moveTo(i16 x, i16 y) { _movePoly(x, y); }

        // Main rotation functions - optimized and consolidated
        // ====================================================
        //
        // setRotation(pitch, yaw, roll) - Sets ABSOLUTE rotation angles for all
        // axes rotate(pitch, yaw, roll) - ADDS rotation amounts to current
        // rotation
        //
        // All parameters default to 0.0f, so you can specify only the axes you
        // want. Examples:
        ...
        void setRotation(float pitch_rad = 0.0f, float yaw_rad = 0.0f,
                         float roll_rad = 0.0f)
        {
            if (!should_rotate)
                return; // Skip rotation if disabled
        ...
        // Add to current rotation values for all axes (rotates by specified
        // amounts)
        void rotate(float pitch_delta = 0.0f, float yaw_delta = 0.0f,
    ...
    protected:
        // Cached rotation matrix elements (arranged for potential SIMD
        // optimization)
        ...
        void _calculateBBox(bool rotating = false)
        {
            ...
            // Find bounds of all rotated points (these are what's actually
            // rendered)
            ...
            for (const auto &p : points_rotated)
            {
                ...
                max_distance_squared =
                    std::max(max_distance_squared, distance_squared);
            }
        ...
        void _calculateRotationVariables(float pitch_val, float yaw_val,
                                         float roll_val)
        {
        ...
        void _calculateRotationVariablesReverse(float pitch_val, float yaw_val,
                                                float roll_val)
        {
            ...
            _calculateRotationVariables(effective_pitch, effective_yaw,
                                        effective_roll);
        }
        ...
        // void _calculateRotationVariables(float pitch_val, float yaw_val,
        // float roll_val, bool reverse = false) {
        ...
        //     if (reverse) {
        //         // Reverse order: Roll -> Yaw -> Pitch (transpose of forward
        //         matrix) _m00 = _yaw_cos * _roll_cos; _m01 = _pitch_sin *
        //         _yaw_sin * _roll_cos + _pitch_cos * _roll_sin; _m02 =
        //         -_pitch_cos * _yaw_sin * _roll_cos + _pitch_sin * _roll_sin;
        //         _m10 = -_yaw_cos * _roll_sin;
        //         _m11 = -_pitch_sin * _yaw_sin * _roll_sin + _pitch_cos *
        //         _roll_cos; _m12 = _pitch_cos * _yaw_sin * _roll_sin +
        //         _pitch_sin * _roll_cos; _m20 = _yaw_sin; _m21 = -_pitch_sin *
        //         _yaw_cos; _m22 = _pitch_cos * _yaw_cos;
        //     } else {
        //         // Forward order: Pitch -> Yaw -> Roll
        //         _m00 = _yaw_cos * _roll_cos;
        //         _m01 = -_yaw_cos * _roll_sin;
        //         _m02 = _yaw_sin;
        //         _m10 = _pitch_sin * _yaw_sin * _roll_cos + _pitch_cos *
        //         _roll_sin; _m11 = -_pitch_sin * _yaw_sin * _roll_sin +
        //         _pitch_cos * _roll_cos; _m12 = -_pitch_sin * _yaw_cos; _m20 =
        //         -_pitch_cos * _yaw_sin * _roll_cos + _pitch_sin * _roll_sin;
        //         _m21 = _pitch_cos * _yaw_sin * _roll_sin + _pitch_sin *
        //         _roll_cos; _m22 = _pitch_cos * _yaw_cos;
        //     }
        ...
        void _applyRotationsToAllPoints()
        {
            ...
            // Apply pre-calculated matrix to all points in optimized loop
            // (inlined for max performance)
        ...
        {
            // Early exit optimization: if no rotation, just copy original
            // points
        ...
        // Apply rotation transformation to a single point (used in addPoint
        // modes)
    ...
    {
        float width, height; // Rectangle dimensions (always positive, float for
                             // sub-pixel precision)
        float calcArea;
        float air_calc;
        float k;
        ...
        Rectangle(float x, float y, float w, float h, const Color<u8> &c,
            ...
            : Polygon(c), width(w), height(h)
        {
            ...
            calcArea =
                RECT_SIM_WIDTH * RECT_SIM_HEIGHT * WORLD_TO_METERS *
                WORLD_TO_METERS; // Calculate area once during construction
            // Initialize physics properties
            ...
            {
                rotate(0.0f, 0.0f,
                       rotation_radians); // Use the new 3-axis rotate function
            }
        }

        // Constructor with just dimensions and color (positioned at origin)
        Rectangle(float w, float h, const Color<u8> &c,
                  float rotation_radians = 0.0f)
        ...
        Rectangle(i16 x, i16 y, u16 w, u16 h, const Color<u8> &c,
                  float rotation_radians = 0.0f)
            : Rectangle(static_cast<float>(x), static_cast<float>(y),
                        static_cast<float>(w), static_cast<float>(h), c,
                        rotation_radians)
        {
        }
        ...
        void resize(float new_width, float new_height)
        {
            ...
            // Restore rotations
            if (current_pitch != 0.0f || current_yaw != 0.0f ||
                current_roll != 0.0f)
            {
                pitch = current_pitch;
                yaw = current_yaw;
            ...
            // Restore position
            moveTo(static_cast<i16>(current_center.x) -
                       static_cast<i16>(width / 2),
                   static_cast<i16>(current_center.y) -
                       static_cast<i16>(height / 2));
        }
        ...
        // Override addPoint to prevent modification (rectangles must stay
        // rectangles)
        ...
        //     // Example: using width and height to calculate a simple drag
        //     coefficient air_calc = 0.5f * AIR_DENSITY * DRAG_COEFF * area; //
        //     Simplified example, can be adjusted
        // }
        void calcAirCalc()
        {
    ...
    private:
        ...
        void _createRectanglePoints(float x, float y, float w, float h)
        {
            ...
            // Add 4 corners: top-left, top-right, bottom-right, bottom-left (no
            // conversions needed!)
